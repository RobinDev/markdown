...
class HeadingTokenTest extends ParserTestCase
{
    ...
    public function test_parse_with_code(): void
    {
        ...
        $this->assertEquals(
            '<h1>Hello, <code class="language-txt">world</code>!</h1>',
            $token->parse(new Parser()),
        );
    }
    ...
    public function test_parse_with_bold_and_italic(): void
    {
        ...
        $this->assertEquals(
            '<h1>Hello, <strong><em>world</em></strong>!</h1>',
            $token->parse(new Parser()),
        );
    }
    ...
    public function test_inline_formatting_rule_priority(): void
    {
        ...
        $this->assertEquals(
            ...
            new HeadingToken('***text***', 1)->parse($parser),
        );
        $this->assertEquals(
        ...
        $this->assertEquals(
            '<h1><strong><em>text</em></strong></h1>',
            new HeadingToken('___text___', 1)->parse($parser),
        );
        $this->assertEquals(
            '<h1><strong>text</strong></h1>',
            new HeadingToken('__text__', 1)->parse($parser),
        );
        $this->assertEquals(
            '<h1><em>text</em></h1>',
            new HeadingToken('_text_', 1)->parse($parser),
        );
    }
}
